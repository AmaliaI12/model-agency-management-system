...
interface HighValueClient {
    NumeClient: string;
    NrContracte: number;
    ValoareMedieContract: number;
}
...
const SidebarLeft: React.FC = () => {
  ...
  const [highValueClients, setHighValueClients] = useState<HighValueClient[]>([]); 
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/api/stats/high-value-clients")
      .then((res) => res.json())
      .then((data) => setHighValueClients(data))
      .catch((err) => console.error("Err high-value-clients:", err));
  }, []);

  // Fetch separat pentru Premium Events (dependent de minBudget)
  const fetchPremiumEvents = () => {
    fetch(
      `http://localhost:5000/api/stats/premium-events?minBudget=${minBudget}`
    )
      .then((res) => res.json())
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <p style={{ fontSize: "0.8rem", color: "#666" }}>
            Earned more than avg model
          </p>
          <ul className="stat-list">
            {modelsAboveAvg.length > 0 ? (
              modelsAboveAvg.map((m, idx) => (
                <li key={idx}>
        ...
        <div className="stat-card">
          ...
          <p style={{ fontSize: "0.8rem", color: "#666" }}>
            {" "}
            10% of total system revenue
          </p>
          <ul className="stat-list">
            {dominantCategories.length > 0 ? (
              dominantCategories.map((c, idx) => (
                <li key={idx}>
        ...
        <div className="stat-card">
          ...
          <ul className="stat-list">
            {premiumEvents.length > 0 ? (
              premiumEvents.map((e, idx) => (
                <li key={idx}>
                  <strong>{e.NumeEveniment}</strong>
                  <br />
        ...
        {/* --- 6. High Value Clients (NOU) --- */}
        <div className="stat-card">
          <strong>High Value Clients</strong>
          <p>Avg contract &gt; Global Active Avg</p>
          <ul className="stat-list">
            {highValueClients.length > 0 ? (
              highValueClients.map((c, i) => (
                <li key={i}>
                  <span>{c.NumeClient}</span>
                  <span>
                    €{c.ValoareMedieContract.toFixed(0)} (x{c.NrContracte})
                  </span>
                </li>
              ))
            ) : (
